...
@section('title', 'Unggah Foto Galeri Baru')
...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="font-playfair text-3xl font-bold text-sogan">Unggah Foto Galeri</h1>
            <p class="text-sm text-gray-500">Unggah foto baru beserta deskripsi singkat untuk galeri website Batikura.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select name="category" id="category" required
                    ...
                    <option value="produk" {{ old('category') === 'produk' ? 'selected' : '' }}>Foto Produk</option>
                    <option value="kemasan" {{ old('category') === 'kemasan' ? 'selected' : '' }}>Foto Kemasan</option>
                    <option value="proses" {{ old('category') === 'proses' ? 'selected' : '' }}>Foto Proses Pembuatan</option>
                </select>
            </div>

            <!-- File Gambar -->
            <div>
                <label for="image" class="block text-sm font-semibold text-dark-brown mb-2">File Gambar (Max 2MB) <span class="text-red-500">*</span></label>
